...
public enum ClaudeWebAPIFetcher {
    ...
    private static func parseUsageResponse(_ data: Data) throws -> WebUsageData {
        ...
        let payloads = self.usagePayloadCandidates(from: json)

        let sessionWindow = self.usageWindow(named: "five_hour", in: payloads)
        let sessionPercent = self.usagePercent(from: sessionWindow)
        let sessionResets = self.usageResetDate(from: sessionWindow)

        let weeklyWindow = self.usageWindow(named: "seven_day", in: payloads)
        let weeklyPercent = self.usagePercent(from: weeklyWindow)
        let weeklyResets = self.usageResetDate(from: weeklyWindow)

        let opusWindow = self.usageWindow(named: "seven_day_opus", in: payloads)
            ?? self.usageWindow(named: "seven_day_sonnet", in: payloads)
        let opusPercent = self.usagePercent(from: opusWindow)
        let extraUsageCost = payloads.compactMap { payload -> ProviderCostSnapshot? in
            guard let extraUsage = payload["extra_usage"] as? [String: Any] else { return nil }
            return self.parseExtraUsageCost(extraUsage)
        }.first
    ...
    private static func usagePayloadCandidates(from root: [String: Any]) -> [[String: Any]] {
        let wrapperKeys = ["usage", "data", "organization", "usage_payload", "usagePayload", "limits"]
    ...
    private static func usageWindow(named key: String, in payloads: [[String: Any]]) -> [String: Any]? {
        payloads.compactMap { $0[key] as? [String: Any] }.first
    }

    private static func usagePercent(from window: [String: Any]?) -> Double? {
    ...
    private static func usageResetDate(from window: [String: Any]?) -> Date? {
        guard let window else { return nil }
        return self.stringValue(window["resets_at"] ?? window["next_reset_at"]).flatMap(self.parseISO8601Date)
    }
