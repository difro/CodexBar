...
struct UsageStoreHighestUsageTests {
    ...
    @Test
    func `Claude provider cost metric participates in highest usage ranking`() {
        let settings = SettingsStore(
            configStore: testConfigStore(suiteName: "UsageStoreHighestUsageTests-claude-provider-cost"),
            zaiTokenStore: NoopZaiTokenStore(),
            syntheticTokenStore: NoopSyntheticTokenStore())
        settings.refreshFrequency = .manual
        settings.statusChecksEnabled = false
        ...
        let claudeSnapshot = UsageSnapshot(
            primary: nil,
            secondary: nil,
            providerCost: ProviderCostSnapshot(used: 40, limit: 100, currencyCode: "USD", updatedAt: Date()),
            updatedAt: Date())
    ...
    @Test
    func `Claude automatic metric falls back to provider cost in highest usage ranking`() {
        let settings = SettingsStore(
            configStore: testConfigStore(suiteName: "UsageStoreHighestUsageTests-claude-automatic-provider-cost"),
            zaiTokenStore: NoopZaiTokenStore(),
            syntheticTokenStore: NoopSyntheticTokenStore())
        settings.refreshFrequency = .manual
        settings.statusChecksEnabled = false
